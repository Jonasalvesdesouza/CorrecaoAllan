...
import { Curso, Disciplina } from "../../interfaces/course.model";
...
// MUDAR O NOME DO CLASS PARA RESULTS
export class ResultShowComponent implements OnChanges {
  ...
  displayResults(): void {
    (document.getElementById("bestChromosomeScore") as HTMLInputElement).value =
      this.resultData.notaDoMaiorCromosso;
  ...
  exportResults(): void {
    const dataStr =
      ...
      encodeURIComponent(JSON.stringify(this.resultData));
  ...
  convertJsonToTable(): void {
    if (this.resultData && this.resultData.cursos) {
      ...
      this.toastr.warning(
        "Nenhum dado de curso disponível para converter.",
        "Aviso"
      );
    }
  }
